<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page import="java.util.ArrayList, com.kh.common.model.vo.PageInfo, com.kh.qna.model.vo.Qna" %>
			...
			<div id="content">
				...
				<div id="admin_contents">
					...
					<div id="qna-search-form">
						<form action="<%= contextPath %>/admin_list.qna?currentPage=1" method="get">
							<select name="keyword">
		            ...
		            <div id="qna-paging-admin" align="center">
					 <% if(keyword != null) { %>		            
		            	...
		            	<% if(currentPage != 1) { %>
		                    <button onclick="location.href='<%= contextPath %>/admin_list.qna?currentPage=<%= currentPage - 1 %>&searchword=<%=searchword %>&keyword=<%=keyword %>';" class="btn btn-secondary">&lt;</button>
		            	<% } %>
		            	
		            	<% for(int p = startPage; p <= endPage; p++) { %>
		            	
		            		<% if(p != currentPage) { %>
		                        <button onclick="location.href='<%= contextPath %>/admin_list.qna?currentPage=<%= p %>&searchword=<%=searchword %>&keyword=<%=keyword %>';" class="btn btn-secondary"><%= p %></button>
		            		<% } else { %>
           			 ...
           			 <% } else { %>		    
           		 		...
           		 		<% if(currentPage != 1) { %>
		                    <button onclick="location.href='<%= contextPath %>/admin_list.qna?currentPage=<%= currentPage - 1 %>';" class="btn btn-secondary">&lt;</button>
		            	<% } %>
		            	
		            	<% for(int p = startPage; p <= endPage; p++) { %>
		            	
		            		<% if(p != currentPage) { %>
		                        <button onclick="location.href='<%= contextPath %>/admin_list.qna?currentPage=<%= p %>';" class="btn btn-secondary"><%= p %></button>
		            		<% } else { %>
		                        <button disabled class="btn btn-secondary"><%= p %></button>
		            		<% } %>
		            	<% } %>
