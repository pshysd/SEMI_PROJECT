...
<style>
	...
	/* ----- 추가한 스타일은 여기에 ----- */
	#news-list-admin tr { height: 30px;}
	#newsTitle:hover {
			...
			<div id="content">
				...
				<div id="admin_contents">
		            ...
		            <div id="news-search-form" >
		                <form action="<%= contextPath %>/admin_list.news?currentPage=1" method="get">
		                    <select name="keyword">
			                    ...
			                    <% } else { %>
			                      <% for(Newsletter ne : list) { %>
			                        <tr style="height: 40px;">
			                          <td><input type="radio"  name="chk" value=<%= ne.getNewsNo() %>></td>
			                          <td><%= ne.getNewsNo() %></td>
			          ...
			          <div id="news-paging-admin" align="center">
			            <% if(keyword != null) { %>
			            	<% if(currentPage != 1) { %>
			                    <button onclick="location.href='<%= contextPath %>/admin_list.news?currentPage=<%= currentPage - 1 %>&searchword=<%=searchword %>&keyword=<%=keyword %>';" class="btn btn-secondary">&lt;</button>
			            	<% } %>
			            	
			            	<% for(int p = startPage; p <= endPage; p++) { %>
			            	
			            		<% if(p != currentPage) { %>
			                        <button onclick="location.href='<%= contextPath %>/admin_list.news?currentPage=<%= p %>&searchword=<%=searchword %>&keyword=<%=keyword %>';" class="btn btn-secondary"><%= p %></button>
			            		<% } else { %>
			                ...
			                <% if(currentPage != maxPage) { %>
			                    <button onclick="location.href='<%= contextPath %>/admin_list.news?currentPage=<%= currentPage + 1 %>&searchword=<%=searchword %>&keyword=<%=keyword %>';">&gt;</button>
			            	<% } %>
			            <% } else { %>
			            	<% if(currentPage != 1) { %>
			                    <button onclick="location.href='<%= contextPath %>/admin_list.news?currentPage=<%= currentPage - 1 %>';">&lt;</button>
			            	<% } %>
			            	
			            	<% for(int p = startPage; p <= endPage; p++) { %>
			            	
			            		<% if(p != currentPage) { %>
			                        <button onclick="location.href='<%= contextPath %>/admin_list.news?currentPage=<%= p %>';" class="btn btn-secondary"><%= p %></button>
			            		<% } else { %>
			                        <button disabled class="btn btn-secondary"><%= p %></button>
			            		<% } %>
			            	<% } %>
			                ...
			                <% if(currentPage != maxPage) { %>
			                    <button onclick="location.href='<%= contextPath %>/admin_list.news?currentPage=<%= currentPage + 1 %>';" class="btn btn-secondary">&gt;</button>
			            	<% } %>
			            <% } %>
			          </div>
